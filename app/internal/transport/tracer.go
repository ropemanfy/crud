...
func (srv *Server) TraceJaeger(serviceName string) *Server {
	...
	trace, srv.reporterCloser, err = cfg.NewTracer(config.Logger(jaegerLog.NullLogger), config.Metrics(metrics.NullFactory))
...
func injectSpan(ctx *fiber.Ctx, span otg.Span) {

	headers := make(http.Header)
	if err := otg.GlobalTracer().Inject(span.Context(), otg.HTTPHeaders, otg.HTTPHeadersCarrier(headers)); err != nil {
		log.Ctx(ctx.UserContext()).Debug().Err(err).Msg("inject span to HTTP headers")
	}
...
func makeSpan(ctx *fiber.Ctx, opName string) (span otg.Span) {
	...
	ctx.SetUserContext(otg.ContextWithSpan(ctx.UserContext(), span))
	return
}
